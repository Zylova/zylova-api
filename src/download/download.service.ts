...
export class DownloadService {
  ...
  constructor(private readonly prisma: PrismaService) {
    this.productsDir = process.env.PRODUCTS_DIR || '/app/products';
  ...
  async saveProductFile(productId: string, file: Express.Multer.File) {
    ...
    if (existing) {
      const oldPath = path.join(this.productsDir, existing.fileName);
  ...
  async deleteProductFile(productId: string) {
    ...
    const fullPath = path.join(this.productsDir, productFile.fileName);
